package com.example.hw10.ioc.adapter;

import com.example.hw10.command.Command;
import com.example.hw10.command.log.LogUnprocessedMethodCommand;
import com.example.hw10.command.operations.UpdateObjectPropertyCommand;
...
public class InitAdapterGeneratorCommand implements Command {
    ...
    public void execute() {
        try {
            ...
            var source = String.format(
                    "package com.example.hw10.operations; public class %s implements %s { %s %s %s }",
                    className,
                    adapterInterface.getName(),
                    "private final com.example.hw10.objects.UObject uObject;",
            ...
            File sourceFile = new File(
                    root,
                    String.format("com/example/hw10/operations/%s.java", className));
            ...
            Class<?> cls = Class.forName(
                    String.format("com.example.hw10.operations.%s", className),
                    true,
                    classLoader);
    ...
    private String getConstructor(String className) {
        return String.format(
                "public %s(com.example.hw10.objects.UObject uObject) { this.uObject = uObject;}",
                className);
    }
    ...
    private String prepareGetMethodBody(String propertyName, Type methodReturnType) {
        return String.format(
                "return java.util.Optional.ofNullable(com.example.hw10.ioc.IoC.<%s>resolve(\"%s.get\", \"%s\", uObject));",
                methodReturnType
                        .getTypeName()
    ...
    private String prepareSetMethodBody(String propertyName, Parameter param) {
        return String.format(
                "com.example.hw10.ioc.IoC.<com.example.hw10.command.Command>resolve(\"%s.set\", \"%s\", uObject, %s).execute();",
                adapterInterface.getSimpleName(),
                propertyName,
                param.getName());
    }
    ...
    private String prepareUnprocessedMethodBody(String propertyName) {
        return String.format(
                "com.example.hw10.ioc.IoC.<com.example.hw10.command.Command>resolve(\"%s.unprocessed\", \"%s\", \"%s\").execute();",
                adapterInterface.getSimpleName(),
                adapterInterface.getSimpleName(),
                propertyName);
    }
